...
export class CombatSystem {
  ...
  resolve(troop, target) {
    ...
    // ── 敵對戰鬥 ──
    // 記錄戰前兵力，用於限制 garrison_regen 回復不得超過戰前值
    const preCombatUnits = target.currentUnits;
    let feedback = null;
    ...
    if (attackPower > defendPower) {
      ...
      let remaining = Math.max(
        0,
        Math.ceil(target.currentUnits - attackPower / effectiveDef)
      );
      ...
      if (target.passiveEffect === 'garrison_regen') {
        ...
        // 回復上限：不得超過戰前兵力（preCombatUnits），也不得超過 maxUnits
        // 確保 garrison_regen 只能補回本次戰鬥損失，不能讓被打後兵力高於戰前
        target.currentUnits = Math.min(
          preCombatUnits,
          Math.min(target.maxUnits, target.currentUnits + regenAmt)
        );
